revoke all on function public.submit_task(uuid, date, text, jsonb) from public, authenticated;
drop function public.submit_task(uuid, date, text, jsonb);
...
begin
  ...
  if not (p_task_id = any(v_plan.required_task_ids) or v_task.schedule = 'optional') then raise exception 'task is not scheduled for this date'; end if;
  if v_task.schedule = 'optional' and exists (
    ...
    where not exists (
      ...
      where child_id = v_child and date_key = p_date_key and task_id = required_id and status in ('completed', 'pending_review')
    )
...
revoke all on function public.submit_task(uuid, date, text, jsonb, timestamptz) from public;
