create function public.correct_recent_checkins(p_start date, p_end date, p_remove date default null)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if p_remove is not null and p_remove >= v_today then
    raise exception 'invalid removal date';
  end if;

  if p_remove is not null then
    update public.daily_plans
      set bonus_awarded = false
      where child_id = v_child and date_key = p_remove;
  end if;
...
revoke all on function public.correct_recent_checkins(date, date, date) from public;
grant execute on function public.correct_recent_checkins(date, date, date) to authenticated;
